...
struct ControlPad: View {
    ...
    let backgroundColor: some ShapeStyle = .gray.tertiary
    ...
    let highlightColor: some ShapeStyle = .tint.tertiary
...
//    }
    ...
    private func movableBounds(totalSize: CGFloat, padding: CGFloat, circleRadius: CGFloat) -> (minX: CGFloat, maxX: CGFloat, minY: CGFloat, maxY: CGFloat) {
        let minX = (padding * 2) + circleRadius / 2
        let maxX = totalSize - (padding * 2) - circleRadius / 2
        let minY =  (padding * 2) + circleRadius / 2
        let maxY = totalSize - (padding * 2) - circleRadius / 2
        return (minX, maxX, minY, maxY)
    }
...
//        }
    ...
    private func updateValues(newLocation: CGPoint, bounds: (minX: CGFloat, maxX: CGFloat, minY: CGFloat, maxY: CGFloat)) {
        ...
        Task { @MainActor in
            ...
            self.xValue = ((newLocation.x - centerX) / (bounds.maxX - centerX - padding)) * 100
            // yValue ranges from -100 to 100.
            self.yValue = ((centerY - newLocation.y) / (centerY - bounds.minY - padding)) * 100
        }
